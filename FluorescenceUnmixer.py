...
uses fluorObjects module to unmix flim data


user input:
    .R64 files of reference dyes and samples
    number of harmonics
    dir of pure dyes
    dir of samples
...
# os.chdir("/Users/taragallagher/Desktop/HyperFluo/")
...
NADH=fluorObjects.FluorObject('NADH')
NADH.add_refFile("/Users/taragallagher/Desktop/Spectral_lifetime_deconvolution/Spectral_LifetimeCh1_passive_2_8ns/pure_solutions/NADH/NADH10mM_8p_fov132_20xA_740nmEx_CH1_460bw80_CH2_540bw50-lp495000$CC0S_ch1_h1_h2.R64")


PYO=fluorObjects.FluorObject('PYO')
PYO.add_refFile("/Users/taragallagher/Desktop/Spectral_lifetime_deconvolution/Spectral_LifetimeCh1_passive_2_8ns/pure_solutions/Pyocyanin/031320_2mMPYO-chamber-01-31-2020_fov39_p5_740nm_20xA_CH1_CH2000$CC0S_ch1_h1_h2.R64")

PVD=fluorObjects.FluorObject("PVD")
PVD.add_refFile("/Users/taragallagher/Desktop/Spectral_lifetime_deconvolution/Spectral_LifetimeCh1_passive_2_8ns/pure_solutions/Pyoverdine/PVD500uM_lifetime_p10_740nm25x08naW_pix16us_ch1_460nm_ch2_540nm_$CC0S_ch1_h1_h2.R64")

LifeSpecies=fluorObjects.LifetimeAsPureComponentSG([NADH,PYO,PVD],1)
...
# for 3 references
LifeTimeRes=np.zeros((3,256,256))


# pixel by pixel
for i in numba.prange(256):
    for j in numba.prange(256):
        LifeTimeRes[:,i,j]= findFractions(LifeSignal[:,:,i,j].reshape(2,1,1), LifeSpecies)
...
res_lifetime=findFractions(LifeSignal,LifeSpecies)
...
#     while ans!='y':
#         plt.hist(np.mean(myob.spectralIm[0],2).flatten()*(np.mean(myob.spectralIm[0],2).flatten()>myob.specthreshold),bins=100,range=[2, np.max(myob.spectralIm[0])])
#         plt.show()
#         myob.specthreshold=np.round(float(input('Where (on x axis) do you want to set threshold')))
#         plt.imshow((np.squeeze(np.mean(myob.spectralIm[0],2))>myob.specthreshold)*np.squeeze(np.mean(myob.spectralIm[0],2)))
#         plt.show()
...
# 
# #Taras plots:
# s,g = ASMPa14coverslip.asSpectralPhasor(1,0)[0].flatten(),ASMPa14coverslip.asSpectralPhasor(1,0)[1].flatten()
# s,g = np.concatenate((s,ASMPa14Nocoverslip.asSpectralPhasor(1,0)[0].flatten())),np.concatenate((g,ASMPa14Nocoverslip.asSpectralPhasor(1,0)[1].flatten()))
# s,g = np.concatenate((s,ASMDphzcoverslip.asSpectralPhasor(1,0)[0].flatten())),np.concatenate((g,ASMDphzcoverslip.asSpectralPhasor(1,0)[1].flatten()))
# s,g = np.concatenate((s,ASMDphzNocoverslip.asSpectralPhasor(1,0)[0].flatten())),np.concatenate((g,ASMDphzNocoverslip.asSpectralPhasor(1,0)[1].flatten()))
# 
# har=2
# 
...
# plt.title('All files \n 8 channels: {} nm - {} nm'.format(ASMPa14coverslip.spectralRange[0][0],ASMPa14coverslip.spectralRange[0][-1]))
...
# 
# circ=plt.Circle((0.,0),radius=1,color='black',fill=False,zorder=10)
# plt.gcf().gca().add_artist(circ)
# axes = plt.gca()
# axes.set_xlim([-1,1])
# axes.set_ylim([-1,1])
...
# 
# 
# x1,mean1=NADH.meanSpectrum(0)[0],NADH.meanSpectrum(0)[1]/(np.max(NADH.meanSpectrum(0)[1]))
# x2,mean2=PYO.meanSpectrum(0)[0],PYO.meanSpectrum(0)[1]/(np.max(PYO.meanSpectrum(0)[1]))
# x3,mean3=PVD.meanSpectrum(0)[0],PVD.meanSpectrum(0)[1]/(np.max(PVD.meanSpectrum(0)[1]))
...
# ax.legend()
# 
# x1,mean1=ASMPa14coverslip.meanSpectrum(0)[0],ASMPa14coverslip.meanSpectrum(0)[1]/(np.max(ASMPa14coverslip.meanSpectrum(0)[1]))
# x2,mean2=ASMPa14Nocoverslip.meanSpectrum(0)[0], ASMPa14Nocoverslip.meanSpectrum(0)[1]/(np.max(ASMPa14Nocoverslip.meanSpectrum(0)[1]))
# x3,mean3=ASMDphzcoverslip.meanSpectrum(0)[0],ASMDphzcoverslip.meanSpectrum(0)[1]/(np.max(ASMDphzcoverslip.meanSpectrum(0)[1]))   
# x4,mean4=ASMDphzNocoverslip.meanSpectrum(0)[0],ASMDphzNocoverslip.meanSpectrum(0)[1]/(np.max(ASMDphzNocoverslip.meanSpectrum(0)[1]))  
#                                         
# fig,ax=plt.subplots()
#                
...
# 
# 
# allsamples=[ASMPa14coverslip,ASMPa14Nocoverslip,ASMDphzcoverslip,ASMDphzNocoverslip]
# resultsSpec=[]
# resultsLife=[]
...
#     a=np.uint16(a)
#     imwrite(os.path.join(os.path.dirname(ob.lifetimefileName[0]),'SpectralResults_NADH_PYO_PVD_NADHb_firstHarmonic.tif'),a,photometric='minisblack') 
...
#     a=np.uint16(a)
#     imwrite(os.path.join(os.path.dirname(ob.SpecfileName[0]),'LifetimeResults_NADH_PYO_PVD_NADHb_firstHarmonic.tif'), a,photometric='minisblack')  
#     i+=1
# 
# i=0
...
# # histogram
#     plt.figure()
#     plt.hist([SpectralRes[i].flatten()[~np.isnan(SpectralRes[i].flatten())] for i in range(4)], bins=15,range=[0, 1], label=['NADH', 'PYO', 'PVD', 'NADH bound'])
#     plt.title(os.path.basename(allsamples[i].SpecfileName[0])[:25]+'\n Spectral results')
#     plt.legend()
#     plt.figure()
#     allsamples[i].showSpecImage(0)
#     
#     H=np.histogram2d(allsamples[i].asSpectralPhasor(1,0)[0].flatten(),allsamples[i].asSpectralPhasor(1,0)[1].flatten(),bins=256,range=[[-1,1],[-1,1]])
#     plt.pcolormesh(H[1], H[2], np.log(H[0]),zorder=0)
#     plt.title(os.path.basename(allsamples[i].SpecfileName[0])[:25]+'\n Spectral results')
...
# # histogram
#     plt.figure()
#     plt.hist([Liferes[i].flatten()[~np.isnan(Liferes[i].flatten())] for i in range(4)], bins=15,range=[0, 1], label=['NADH', 'PYO', 'PVD', 'NADH bound'])
#     plt.title(os.path.basename(allsamples[k].lifetimefileName[0])[:20]+'\n Lifetime results')
#     plt.legend()
#     plt.figure()
...
#     k+=1    
#     
# plt.hist(SpectralRes[0].flatten()[~np.isnan(SpectralRes[0].flatten())],bins=50,range=[0, 1],label='spectralNADH')
# plt.title('Fractional Contribution - Spectral \n NADH')
# plt.legend()
# 
# plt.figure()
# plt.hist(LifeTimeRes[0].flatten()[~np.isnan(LifeTimeRes[0].flatten())],bins=50,range=[0, 1],label='lifetimeNADH')
# plt.title('Fractional Contribution - LifeTime \n NADH')
# plt.legend()
# 
# plt.figure()
# plt.hist(SpectralRes[1].flatten()[~np.isnan(SpectralRes[1].flatten())],bins=50,range=[0, 1],label='spectralPYO')
# plt.title('Fractional Contribution - Spectral \n PYO')
# plt.legend()
# 
# plt.figure()
# plt.hist(LifeTimeRes[1].flatten()[~np.isnan(LifeTimeRes[1].flatten())],bins=50,range=[0, 1],label='lifetimePYO')
# plt.title('Fractional Contribution - LifeTime \n PYO')
# plt.legend()
# 
# plt.figure()
# plt.hist(SpectralRes[2].flatten()[~np.isnan(SpectralRes[2].flatten())],bins=50,range=[0, 1],label='spectralPVD')
# plt.title('Fractional Contribution - Spectral \n PVD')
# plt.legend()
# 
# plt.figure()
# plt.hist(LifeTimeRes[2].flatten()[~np.isnan(LifeTimeRes[2].flatten())],bins=50,range=[0, 1],label='lifetimePVD')
# plt.title('Fractional Contribution - LifeTime \n PVD')
# plt.legend()
# 
# plt.figure()
# plt.hist(SpectralRes[3].flatten()[~np.isnan(SpectralRes[3].flatten())],bins=50,range=[0, 1],label='spectralPVD')
# plt.title('Fractional Contribution - Spectral \n PVD')
# plt.legend()
# 
# plt.figure()
# plt.hist(LifeTimeRes[3].flatten()[~np.isnan(LifeTimeRes[3].flatten())],bins=50,range=[0, 1],label='lifetimePVD')
# plt.title('Fractional Contribution - LifeTime \n PVD')
# plt.legend()
# 
# 
...
# 
#     
#     imwrite(os.path.join(os.path.dirname(myob.lifetimefileName[0]),'SpectralResults_NADH_PYO_PVD_NADHb_firstHarmonic.tif'), np.uint16(1000*SpectralRes))
#     imwrite(os.path.join(os.path.dirname(myob.SpecfileName[0]),'LifetimeResults_NADH_PYO_PVD_NADHb_firstHarmonic.tif'), np.uint16(1000*LifeTimeRes))  
