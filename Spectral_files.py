...
os.chdir("/Volumes/GoogleDrive/My Drive/PseudomonasImaging/Data/Spectral/TCEP_gradients")
...
fig=sns.relplot(x="Wavelength", y="Intensity", kind="line", hue="TCEP_conc", data=spec_df)
...
sample_names_df=sample_names_df.rename(columns={0:"entry1", 1:'entry2', 2:'entry3'})
sample_names_df.insert(0, "NewSample", sample_names_df["entry1"] + sample_names_df["entry2"])
spec_df2=pd.concat([spec_df, sample_names_df], axis=1)
spec_df_tcep=spec_df2[spec_df2.Sample.str.contains(pat="TCEP") & spec_df.Sample.str.contains(pat="PYO")]
spec_df_tcep.insert(0, "TCEP", spec_df_tcep.entry2.str.split(pat="mm", expand=True)[0])
spec_df_tcep.TCEP.replace("umTCEP", "", regex=True)


spec_df_tcep=spec_df_tcep.loc[(spec_df_tcep["TCEP"]=="100umTCEP") | (spec_df_tcep["TCEP"]=="400umTCEP") | (spec_df_tcep["TCEP"]=="500umTCEP") | (spec_df_tcep["TCEP"]=="125") | (spec_df_tcep["TCEP"]=="2")]

spec_df_tcep.insert(0, "containsmicromolar", spec_df_tcep["TCEP"].str.contains("um", regex=True))
spec_df_tcep["test"]=np.where(spec_df_tcep.containsmicromolar==True, spec_df_tcep.TCEP.replace("umTCEP", "", regex=True).astype('int') /1000, spec_df_tcep.TCEP)
spec_df_tcep["TCEP_conc"] = spec_df_tcep['test'].astype('str')
spec_df_tcep["TCEP_conc"] = spec_df_tcep['TCEP_conc'].astype('category')

fig, ax = plt.subplots()
flatui = ["#9b59b6", "#3498db", "#95a5a6", "#e74c3c", "#34495e"]
g=sns.lineplot(x="Wavelength", y="Intensity",  hue="TCEP_conc", hue_order=['125', '2', '0.5', '0.4', '0.1'],  style="TCEP_conc", ci=None, data=spec_df_tcep, palette=flatui, legend=False)
...
for j in range(1,20,2):
    ...
    for i in range(0,400,2):
       ax[1].cla()
       a=np.abs(ft.ifft(ft.fft(sp)*ft.fftshift(np.exp(1j*(np.linspace(0,i,32))))))
