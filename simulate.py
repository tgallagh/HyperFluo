...
def testAlgo(f, taus, har):
    ...
    plt.plot(
        ...
        signal[1, 0],
        "y2",
        label="Sig:f1=%1.3f,f2=%1.3f,f3=%1.3f,f4=%1.3f" % (f[0], f[1], f[2], f[3]),
    )
    ...
    plt.plot(
        ...
        comboPredict[1, 0],
        "r 1",
        label="Calc.:f1=%1.3f,f2=%1.3f,f3=%1.3f,f4=%1.3f"
...
def genImage(tau, sgnoise, sz, har, binning):
    ...
    for i in range(0, har):
        H, xedges, yedges = np.histogram2d(
            np.reshape(im[:, :, 2, 0], 256 * 256),
            np.reshape(im[:, :, 1, 0], 256 * 256),
            bins=(xedges, yedges),
        )
        H = H.T
...
saveres = (
    []
)  # in the form of [fractional contributions,  image location xy] => [0..n, x,y]
d = 0
signal = np.zeros((2, har))
...
myfile = (
    "031320_PA14chunk_nocoverslip_fov55_p7_780nm_20xA_CH1_CH2000$CC0S_ch1_h1_h2.R64"
)
f = lfd.SimfcsR64(myfile)
with lfd.SimfcsR64(myfile) as f:
    x = np.reshape(
        f.asarray()[2, :, :] * np.sin(np.deg2rad(f.asarray()[1, :, :])), 256 * 256
    )
    y = np.reshape(
        f.asarray()[2, :, :] * np.cos(np.deg2rad(f.asarray()[1, :, :])), 256 * 256
    )
    H, xedges, yedges = np.histogram2d(x, y, bins=(xedges, yedges))
    H = H.T
    ax.imshow(
