...
class FluorObject:
    ...
    def info(self):
        if len(self.spectralIm) > 0:
            print(
                "{} has {} Spectral (lsm) images. \n \n Filename and path = {}".format(
                    self.name, len(self.spectralIm), self.SpecfileName
                )
            )
        if len(self.lifetimeIm) > 0:
            print(
                "\n Object {}, has {} Lifetime images. \n \n Filename and path = {}".format(
                    self.name, len(self.lifetimeIm), self.lifetimefileName
                )
            )
        ...
        if os.path.splitext(LSMfile)[1] == ".lsm":
            ...
            try:
                self.spectralRange.append(
                    np.asarray(ti.lsm_metadata["ChannelColors"]["ColorNames"]).astype(
                        float
                    )[:ch]
                )
            except ValueError:
    ...
    def add_refFile(self):
        ...
        RefFile = tkinter.filedialog.askopenfilename(
            title="Select a .r64 file", filetypes=[("R64 files", "*.r64")]
        )
...
def findFractionsMLE(signal, species, var):
    ...
    res = minimize(
        funcFs, init, method="SLSQP", bounds=bnds, constraints=cons, tol=1e-10
    )
...
def findFractions(signal, species):
    ...
    def funcFs(x):
        ...
        chisq = np.sum(np.sum((signal[0] - np.sum(x * species[0], 1)) ** 2)) + np.sum(
            np.sum((signal[1] - np.sum(x * species[1], 1)) ** 2)
        )
        return chisq
    ...
    }  # , 'jac': lambda x:np.array(1.0 for x in range(sz))})
    # res = minimize(funcFs,init , method='SLSQP',bounds=bnds,constraints = cons,tol=1e-10)
    # res = minimize(funcFs,init , method='Nelder-Mead')#,bounds=bnds)#,constraints = cons,tol=1e-10)
...
def findFractionsinit(signal, species, init):
    ...
    def funcFs(x):
        chisq = np.sum(
            np.sum((np.squeeze(signal) - np.squeeze(np.sum(x * species, 2))) ** 2)
        )
        return chisq
    ...
    res = minimize(
        funcFs, init, method="SLSQP", bounds=bnds, constraints=cons, tol=1e-7
    )
...
def findFractionsComplete(specsignal, specspecies, lifesignal, lifespecies):
    ...
    def funcFs(x):
        chisq = np.sum(
            np.sum(
                (np.squeeze(lifesignal) - np.squeeze(np.sum(x * lifespecies, 2))) ** 2
            )
        ) + np.sum(
            np.sum(
                (np.squeeze(specsignal) - np.squeeze(np.sum(x * specspecies, 2))) ** 2
            )
        )  # spectrum
        return chisq
...
def GiveMeAHyperSignal(wavearray, signal, lambdaStart, lambdaEnd, channels):
    ...
    if chans < channels:
        print(
            "You're asking for too many channels ({}), defaulting to max possible ({})".format(
                channels, chans
            )
        )
        ...
        if windows < channels:
            sigout[-1] = np.mean(
                signal[i + (l) * channels : i + (l + 2) * windows]
            )  ##Not sure
